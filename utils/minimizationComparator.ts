...
const evaluateSOP = (
  ...
  return terms.some((term) => {
    const clean = term.replace(/[()]/g, "").replace(/[·*]/g, "").replace(/\s+/g, "");
...
const areEquivalentOnDefinedCells = (
  ...
  for (let index = 0; index < values.length; index++) {
    ...
    const a = evaluateExpression(firstExpression, type, variableQuantity, index);
    const b = evaluateExpression(secondExpression, type, variableQuantity, index);
    if (a !== b) {
      return false;
    }
  }
...
const solveExact = (
  ...
  const dedupExpressions = [...new Set(
...
const solveHeuristic = (
  ...
  while (uncovered.size > 0) {
    ...
    implicants.forEach((candidate) => {
      const newlyCovered = candidate.covers.filter((value) => uncovered.has(value))
        .length;
      if (newlyCovered === 0) {
        return;
      }
  ...
  const minimized = selected.filter((pattern, index) => {
    ...
    others.forEach((candidatePattern) => {
      const implicant = implicants.find((item) => item.pattern === candidatePattern);
...
export const buildMinimizationComparison = (
  ...
  const heuristic = solveHeuristic(variableQuantity, resultType, minterms, dontCares);
